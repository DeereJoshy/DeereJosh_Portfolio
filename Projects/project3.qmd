...
```{python}
q3 = '''
SELECT playerID, SUM(AB), SUM(CAST(H AS FLOAT)) / SUM(CAST(AB AS FLOAT)) * 100 AS 'battingAverage%'
FROM Batting
GROUP BY playerID
HAVING SUM(AB) >= 100
...
fig = px.scatter(title= 'The Yankees and The Sox' , x = 'yearID', y = 'W/L Ratio')
```